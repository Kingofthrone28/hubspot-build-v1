$(() => {
  ...
  const pieChart = raphael.piechart(radius, radius, radius, slicesConfig, {
    strokewidth: 2,
  });
  ...
  function addSlice(index) {
    ...
    for (const [categoryIndex, category] of categories.entries()) {
      const { field, name } = category;

      if (categoryIndex === index) {
        $canvas.prepend(
          $(
            `<span class="${field} slice" data-field="${field}" data-name="${name}" data-id="${categoryIndex}"><div class="inner">${name}</div></span>`,
          ),
        );
