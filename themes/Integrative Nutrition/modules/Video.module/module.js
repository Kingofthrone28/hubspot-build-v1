...
function loadOEmbed(container) {
  ...
  const iframeModalOverlay = document.querySelectorAll('.video-modal .modal-overlay');
  const modalDialog = document.querySelector('.video-modal .modal-dialog');
  const modalDialogClose = document.querySelectorAll('.video-modal .modal-dialog .modal-close');
      ...
      if (request.status >= 200 && request.status < 400) {
        ...
        iframeModalOverlay.forEach((item) => {
          item.onclick = function () {
            this.closest(".oembed_modal_container").getElementsByTagName('button')[0].classList.remove('oembed_custom-thumbnail--hide');
